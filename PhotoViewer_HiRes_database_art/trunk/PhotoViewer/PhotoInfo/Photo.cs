...
namespace PhotoViewer.PhotoInfo
{
    public class Photo : IEnumerable
    {
        ...
        public bool equalTag(String tag)
        {
            foreach (string s in ptag.allTags)
                if (s.Equals(tag, StringComparison.OrdinalIgnoreCase))
                    return true;
            return false;
        }
        ...
        Vector2 maxTagSize = Vector2.Zero;

        
        public void ShowMetadata()
        {
            maxTagSize = ptag.drawTags((int)(this.PositionDisplay.X + this.WidthDisplay / 2 + Browser.MAR), (int)(this.PositionDisplay.Y - this.HeightDisplay / 2), angleDisplay_, (int)HeightDisplay, ScaleDisplay);
        }
        ...
        public void postProcess()
        {
            ...
            if (!IsGazeds && oldGazed)
                ptag.hideTag();
            if (!oldGazed && IsGazeds)
                ptag.showTag();
            oldGazed = IsGazeds;
        }
        ...
        public void Render(SpriteBatch batch, bool isFiltering, bool isShadowing, Texture2D textureShadow,bool isWhiteframe, bool isDragged, Texture2D textureFrame, bool isFavorite)
        {
            if (isFiltering)
            {
                ...
                if (ID == 1)
                {
                    Console.WriteLine(positionTarget_);
                    Console.WriteLine(positionDisplay_);
                    Console.WriteLine(velocityDisplay_);
                }
            }
            else
            {
