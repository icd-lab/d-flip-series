...
namespace PhotoViewer.PhotoInfo.Tag
{
    public class ArtworksTag : PhotoTag
    {
        public class TagEntity
        {
            ...
            public TagEntity(string from, PhotoTag tags)
            {
                Name = from;
                this.tags = tags.allTags;
                StartYear = tags.startDate;
                EndYear = tags.endDate;
            }
        }
        public class ColorEntity
        {
            ...
            public ColorEntity(string from, Photo.colorFeature color)
            {
                Name = from;
        ...
        public static Dictionary<string, Photo.colorFeature> FromColorXml(string xml)
        {
            XmlSerializer ser = new XmlSerializer(typeof(ColorEntity[]));
            var tags = (ColorEntity[])ser.Deserialize(new StringReader(xml));
        ...
        public static Dictionary<string, PhotoTag> FromXml(string xml)
        {
            ...
            Dictionary<string, PhotoTag> Features = new Dictionary<string, PhotoTag>();
            foreach (var tag in tags)
            {
                Features[tag.Name] = new ArtworksTag(tag.tags, tag.StartYear, tag.EndYear);

            }
            return Features;
        }

        public static string ExportXml(Dictionary<string, PhotoTag> colorFeatures)
        {
        ...
        //public static string ExportColorXml(Dictionary<string, Photo.colorFeature> colorFeatures)
        //{
        //    List<ColorEntity> list = new List<ColorEntity>();
        //    foreach (var t in colorFeatures.Keys)
        //    {
        //        list.Add(new ColorEntity(t, colorFeatures[t]));
        //    }
        //    XmlSerializer ser = new XmlSerializer(typeof(ColorEntity[]));
        //    StringWriter writer = new StringWriter();
        //    ser.Serialize(writer, list.ToArray());
